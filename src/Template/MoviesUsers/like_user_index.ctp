...
<div class="row justify-content-around user-card-area">
  <?php foreach ($likeUsers as $user) : ?>
    <div class="like-user-area p-4 mb-5">
      <div class="row">
        <div class="col-4">
          <?= $this->Html->image($user->profile_image_url,['class'=>'like-user-image mb-3','width'=>'100%','alt'=>'プロフィール画像']) ?>
          ...
          <input type="hidden" class="like-user-hidden-friends-count" value="<?= $user->friends_count ?>">
          <input type="hidden" class="like-user-hidden-followers-count" value="<?= $user->followers_count ?>">
          <input type="hidden" class="like-user-hidden-other-url" value="<?= $user->other_url ?>">

        </div>
        <div class="col-8 like-user-area-info">
          <div class="font-weight-bold border-bottom border-success">名前</div>
          <div class="name mt-2"><?= $user->name ?></div>
          <div class="font-weight-bold border-bottom border-success mt-3">自己紹介</div>
          <div class="introduce mt-2"><?= $user->description ?></div>
        </div>
      </div>
    </div>
  <?php endforeach ?>
